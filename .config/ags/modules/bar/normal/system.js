...
const BarBattery = () => Box({
    ...
    children: [
     ...
     setup: (self) => self.hook(Battery, BarBattery => {
        const end = GLib.DateTime.new_from_unix_utc(Battery.time_remaining); 
        const timeString = end.format_iso8601().substring(11,19);
        
        if (Battery.charging) {
            BarBattery.tooltipText = `${timeString} until fully charged`
        } else {
            BarBattery.tooltipText = `${timeString} until fully depleted`
        }
    }),
});
